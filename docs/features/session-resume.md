...
启动
  ...
  └─ send(prompt)
      ...
      │      → 清空 sessionId + 重试一轮 + onSessionFallback()
      │
...
`isResumeFailure(msg)` 检查 SDK 消息中 `subtype` / `errors` 含 "session" + "not found/expired/invalid"。匹配时：

1. 清空 `lastSessionId` + `clearSessionId(file)`
2. 触发 `callbacks.onSessionFallback(previousSessionId)`
3. 用 `undefined` sessionId 重发一轮 prompt（新开 session）

### 3. UI 提示

`main.ts` 中 `onSessionFallback` 回调打印：
...
`tests/session.test.ts`（8 个）：流式文本、错误子类型、工具调用、状态流转、危险工具回调等。

### 手动验证

```powershell
# 1. 启动 VoxBridge，发一句话（生成 session）
node dist/main.js
> "你好"
...
# 5. 伪造 resume 失败：把 sessionId 改成不存在的 "invalid-sess-xyz"
# 发一句话 → 应看到 "会话续接失败（ID=invalid-…），已开启新会话"
```

## 故障排查

**会话不恢复**：检查 `~/.voxbridge-session.json` 中 `updatedAt` 是否在 7 天内。超过会被自动丢弃。

**resume 反复失败**：可能是 Claude Agent SDK 端会话已被清理（服务端 30 天上限）。看 `~/.VoxBridge/logs/` 中 `resume failed` warn 日志。
