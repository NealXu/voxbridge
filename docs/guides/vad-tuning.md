...
**silero-vad 加载失败**：worker 发 `{"type":"ready","vad":"energy_threshold"}` 表示退化到能量阈值。检查 `~/.VoxBridge/logs/` 错误日志。

**参数没生效**：检查 `logs/worker.log` 中 `spawning worker` 行应有 `--vad-threshold 0.45 ...` 字样。
