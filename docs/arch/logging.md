...
voxcode 使用结构化日志系统，替代原先硬编码的 `voxcode-debug.log`（已移除）。日志按**级别** / **命名空间** / **时间**三维组织，便于生产环境排查与优化。

**关键特性**：
- 5 级日志：`debug` / `info` / `warn` / `error` / `fatal`
- 命名空间：`main` / `stt.worker` / `executor` / `session` / `trigger.*`
- 异步写盘：内存队列 + 单 writer 协程，永不阻塞业务线程
...
**环境变量覆盖**（优先级高于配置文件）：
- `VOXCODE_LOG_LEVEL=debug`：覆盖级别
- `VOXCODE_LOG_CONSOLE=1`：强制开启控制台输出
...
- 默认：`~/.voxcode/logs/voxcode.YYYY-MM-DD.log`
- 轮转后：`~/.voxcode/logs/voxcode.YYYY-MM-DD.N.log`（N 为序号）
...
```powershell
Get-ChildItem "$env:USERPROFILE\.voxcode\logs\*.log" | Where-Object LastWriteTime -lt (Get-Date).AddDays(-30) | Remove-Item
```
...
```bash
VOXCODE_LOG_CONSOLE=1 VOXCODE_LOG_LEVEL=debug npm start
```

---

## 8. 迁移说明（从 voxcode-debug.log）

旧版本（<0.3.0）在当前目录生成 `voxcode-debug.log`（同步写盘，无轮转）。新版本：

- ❌ 不再写入 `./voxcode-debug.log`
- ✅ 改为 `~/.voxcode/logs/voxcode.YYYY-MM-DD.log`
- ✅ 异步写盘，无性能影响
- ✅ 自动轮转，无需手动清理
...
```powershell
Remove-Item ./voxcode-debug.log -ErrorAction SilentlyContinue
```
