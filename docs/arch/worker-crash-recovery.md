...
> P1-1：模拟 worker 进程异常退出，验证自动重启与 3 次上限退出。
...
`main.ts` 维护环形缓冲（最近 20 行），崩溃时截取最后 5 行写入日志：
...
`handleWorkerExit` 在创建新 client 前先 `await stt.dispose()`，确保旧 worker 子进程被回收，避免 `python.exe` 僵尸。
...
崩溃事件写入 `~/.voxcode/logs/YYYY-MM-DD.log`：
...
**僵尸 python.exe**：说明 `dispose()` 未成功回收。检查日志 `worker exit` 事件，`intentional` 应为 true。

**重启卡住**：检查 `waitReady` 60s 超时是否命中。可能是模型加载失败，看 `lastStderr`。

**3 次连续崩溃**：通常是模型损坏或 OOM。看 fatal 日志的 `lastStderr`，必要时重建模型目录。
