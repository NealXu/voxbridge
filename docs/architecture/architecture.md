...
旧版 VoxBridge 直接调用 AI API，没有利用 Claude Code 的完整能力：
...
**VoxBridge 作为语音控制器，启动并控制本地 Claude Code 实例执行 coding 任务。**
...
VoxBridge 采用**多进程架构**：Node.js 主进程作为语音控制器，通过 claude-agent-sdk 启动并控制本地 Claude Code 实例执行 coding 任务；Python 子进程负责语音采集和识别。
...
| **环境变量** | `src/env.ts` | 读取 ~/.claude + .claude 下 env 块 |
...
npm 依赖：
  ...
  node-pty ──────────────────────── PTY 模式（P3，可选 prebuild）
```
...
API Token            只存 ~/.claude/settings.json
                     cc 子进程直接读取，VoxBridge 不处理
语音数据             仅存于内存，不落盘，不传输
                     （本地 Whisper 推理）
会话 ID              存 ~/.voxbridge-session.json
                     仅包含 session UUID
代码/文件            cc 在 cwd 内操作
                     继承 Claude Code 权限模型
```
...
| 命令执行 | `bypassPermissions` 模式，语音确认即执行 |
| API 调用 | cc 通过 HTTPS 直连 API，token 不经 VoxBridge |
...
│     (Windows Terminal/PowerShell 为 ESC[20~)   │
│   - 300ms 防抖：防止快速连续触发               │
│   - Esc 取消录音                               │
│   - Ctrl+C 优雅退出                            │
└────────────────────────────────────────────────┘
...
日志文件位于项目根目录 `VoxBridge-debug.log`，可安全删除。
...
│  ├── session/                  Agent 会话
│  │  ├── agentSession.ts       会话管理器（由 executor 驱动）
...
1. **实现 ExecutorRegistry** → 管理 cc 进程池，每个 chatId 映射
