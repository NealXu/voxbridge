...
将 VoxBridge 从「直接调用 AI API」重构为「通过 claude-agent-sdk 启动并控制本地 Claude Code 实例」，获得完整 coding 能力（读写文件、运行命令、调试、Agent Teams）。

**范围**（2026-08-14/15 实施完成）：
- 新增 `src/executor/`（共 16 模块 + pty/ 3 文件）：ClaudeExecutor、StreamProcessor、AsyncQueue、envFilter、claudePath、costTracker、crashRecovery、errors、registry、persistentExecutor、teamState、teamHooks、types、index、ptySession、screenWatcher
...
|---|---|---|---|---|---|
| **P0** | 基础 SDK 模式 | 无 | `src/executor/` 核心 + 单次执行 | 语音→cc→文件修改全链路 | ✅ |
| **P1** | 会话持久化 + UI 完整性 | P0 | sessionManager + UI 增强 | 跨重启续接；工具/文件/成本显示 | ✅ |
...
通过 `@anthropic-ai/claude-agent-sdk` 启动 cc 子进程，实现一次完整的语音→编码任务执行流。
...
| P1-5 | ink TUI 增强 | `src/ui/ink/OutputPanel.tsx` | 工具调用流 |
...
└────────────────────────────────────────────────────┘
┌─ 完成统计 ─────────────────────────────────────────┐
│ 耗时 12.3s │ 成本 $0.02 │ 3 turns                 │
└────────────────────────────────────────────────────┘
```

### 3.4 验收标准

- [ ] 删除 `~/.voxbridge-session.json` 后重启 → 新会话
...
- 团队命名必须用 \`VoxBridge-<chatId>\`-\` 前缀，避免冲突
...
ExecutorRegistry
    ...
    ├─ acquire(chatId)
    │  ├─ 存在 → 刷新空闲计时 → 返回
    │  └─ 不存在 → spawn 新 cc 进程 + 注入 Team 钩子 → 返回
    │
    ├─ 运行中
...
- [ ] 同一 chatId 连续语音 → 复用同一 cc 进程
- [ ] 语音指令「创建一个 5 人团队帮我并行重构」→ 5 个队友
- [ ] UI 显示 teammates（名称 + 状态）和 tasks
- [ ] 第 11 个队友 → 拒绝创建，提示上限
- [ ] 空闲 30min → 进程释放
...
```bash
npm test                          # Node 全部测试
npm test -- --test-name-pattern="executor"   # 单文件测试
...
| 风险 | 概率 | 影响 | 缓解 |
|---|---|---|---|
| SDK API 变更 | 低 | 高 | 锁定版本；隔版 SDK 调用测试 |
| cc 版本兼容 | 中 | 高 | 最低版本检查；错误提示升级 |
| 进程泄漏 | 中 | 高 | 空闲回收 + dispose 兜底 + 测试 |
| Agent Teams API 不稳定 | 高 | 中 | 渐进启用；降级为普通执行 |
| 队友并行冲突 | 中 | 中 | in-process 模式；命名前缀隔离 |
| 性能（多 cc 进程内存高） | 中 | 中 | maxConcurrent 限制 + 空闲回收 |
...
| **AsyncQueue** | `src/executor/inputQueue.ts` | 多轮输入队列，支持 sendAnswer / 并发消费流 |
...
| # | 问题 | 状态 | 说明 |
|---|---|---|---|
| 1 | **PTY 模式未实现**（P3） | ✅ 已实现 | `src/executor/pty/`：node-pty 驱动真实 claude TUI，28 测试 |
| 2 | **PersistentExecutor 仍 Scaffold** | ✅ 已完成 | 完整消费循环：turn 分类、spontaneous/continuation-turn/between-turn-question 事件、nextTurn/shutdown |
| 3 | **Agent Teams 需端到端实现** | ✅ 集成测试 | `tests/claudeIntegration.test.ts`（`CLAUDE_INTEGRATION=1` 才跑，默认 skip） |
| 4 | **tsc 预存类型错误（3 处）** | ✅ 已修复 | 新增 `@types/ws`、MockFn 类型、exitReason 类型修复；tsc 0 错误 |
| 5 | **ink 渲染 arity 错误** | ✅ 已修复 | 先前提交 `fbc8842` 已修复（1 处 render 调用），验证无残留 |
| 6 | **user-guide.md 未更新** | ✅ 已更新 | executor/claude 配置 + Agent Teams 用法（v0.2.0） |
| 7 | **端到端语音验证待办** | ✅ 验证脚本 | `scripts/verify-e2e.ps1` + `docs/arch/e2e-verification.md`（需麦克风环境手动执行） |
